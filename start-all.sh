...
echo -e "${BLUE}  Микросервисная архитектура${NC}"
echo -e "${BLUE}=================================${NC}\n"
...
echo -e "${BLUE}Основные эндпоинты (через Backend Service):${NC}"
echo -e "  GET  /health            — статус backend"
echo -e "  POST /upload_model      — загрузка 3D-модели"
echo -e "  GET  /projects          — список проектов"
echo -e "  POST /generate_furniture — генерация мебели\n"

echo -e "${BLUE}Авторизация (Auth Service):${NC}"
...
cleanup() {
    ...
    kill $AUTH_PID $ML_PID $BACKEND_PID $FRONTEND_PID 2>/dev/null
